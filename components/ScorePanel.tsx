  ...
  return (
    <aside className="rounded-2xl border border-white/9 bg-white/[0.025] p-5 shadow-[0_18px_80px_rgba(0,0,0,0.28)]">
      <div className="flex items-start justify-between gap-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <Link
              href="/guide#score"
              aria-label="Production Fit 점수 읽는 법"
              title="점수 읽는 법"
              className="grid size-5 place-items-center rounded-full border border-white/10 text-[10px] font-semibold text-zinc-500 transition hover:border-amber-200/25 hover:text-amber-100"
            >
              ?
            </Link>
          </div>
      ...
      <div className="mt-6 space-y-4">
        ...
        <Metric label="포화도 안전성" value={score.saturation} max={20} />
      ...
      <div className="mt-5 rounded-xl border border-white/7 bg-black/25 p-3 text-xs leading-5 text-zinc-500">
        위 점수는 리포트 원문 수치가 아니라 MV Trend Studio가 제작 의사결정을 위해 산정한 제품 점수입니다. <Link href="/guide#score" className="font-medium text-zinc-300 transition hover:text-amber-100">점수 읽는 법 →</Link>
      </div>
    </aside>
  );
}
